...
The format is based on [Keep a Changelog](http://keepachangelog.com/) and the versioning follows the [Semantic Versioning 2.0.0](https://semver.org/) specification.
...
- Improved IPv6 support: added `vcn_is_oracle_gua_allocation_enabled`, `vcn_ipv6private_cidr_blocks`, and `vcn_byoipv6cidr_details` variables
- Added dual-stack route table `nat_ipv4_igw_ipv6` for NAT (IPv4) + Internet Gateway (IPv6)
- Added subnet `route_table_id` override and `prohibit_internet_ingress` support
- Added IPv6 rules (egress, SSH, ICMPv6) to the VCN Default Security List when `enable_ipv6 = true`
